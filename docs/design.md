...
- Role matrix on Coupon write surfaces: every commerce-mutating coupon endpoint (`POST /coupons`, `POST /coupons/:code/claim`, `POST /coupons/:code/redeem`, `POST /coupons/:id/distribute`, `POST /coupons/:id/expire`) is restricted to `store_admin` and `platform_admin` only. **Both `auditor` AND `content_reviewer` are denied with 403** — auditor for being read-only, content_reviewer because the review role is scoped to question/paper QA and granting it commerce-mutation rights would create an unbounded discount surface the business model never authorised. `store_admin` callers are additionally restricted to coupons in their own store via the hiding policy (foreign-store coupons return 404, not 403).
